...
class SmsHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        session = self.settings['redis'].get(sockmap[otp]['_xsrf'])
        if not session:
            self.set_status(400)
            self.finish()
            return
        ...
        self.settings['redis'].set(sockmap[otp]['_xsrf'],pickle.dumps(session))
        del sockmap[otp]
        self.set_status(200)
        self.finish()
        return

known_types = ['email','eotp','ssms','addr']
...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
    def on_close(self):
        for k,v in sockmap.items():
            if v['websock'] == self:
                self.settings['redis'].delete(v['_xsrf'])
                del sockmap[k]
                tornado.log.gen_log.info('removed sock ' + str(v))
                break
    ...
    def on_message(self,message):
        ...
        _xsrf = self.get_cookie('_xsrf')
        if 'type' not in data or data['type'] not in known_types or '_xsrf' not in data or _xsrf != data['_xsrf']:
            self.close(code=1003,reason='invalid message type')
        res = {}
        if data['type'] == 'email':
            # generate a otp using hotp and email as key, send confirmation mail with otp
            ...
            res['type'] = "eotp"
            session = {'actual': data['email'], 'eotp': '123456'}
            self.settings['redis'].set(data['_xsrf'],pickle.dumps(session))
            self.write_message(json.dumps(res))
            return
        elif data['type'] == "eotp":
            session = self.settings['redis'].get(data['_xsrf'])
            if not session:
                self.close(code=1003,reason='Invalid Session')
            session = pickle.loads(session)
            if session['eotp'] == data['eotp']:
                imgdata = yield self.settings['tpool'].submit(gen_data,session['eotp'])
                res['innermodal'] = "<h5 class='w3-center'>SMS READDR " + session['eotp'] + " to 9988776655</h5><br><h5 class='w3-center'>OR</h5><br><div class='w3-container w3-hide-small w3-center'><h5>Scan the below QRCODE</h5><img class='w3-image' style='max-height: 150px;' src='" + imgdata + "'/></div><div class='w3-container w3-hide-medium w3-hide-large w3-center'><a href='smsto:9988776655:" + session['eotp'] + "'>Click to Send</a></div><h5 class='w3-center'>The page will auto-update as soon as we receive your SMS</h5>"
                res['modalhead'] = "<h5>Send SMS to Add Phone Number</h5>"
                res['type'] = "ssms"
                sockmap[session['eotp']] = {'websock': self, '_xsrf': data['_xsrf']}
                self.write_message(json.dumps(res))
            else:
            ...
            self.settings['redis'].delete(data['_xsrf'])
